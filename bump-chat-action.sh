#!/bin/sh
# Bump version for chat-action package
# Usage: ./bump-chat-action.sh [major|minor|patch]
...
PACKAGE_DIR="packages/chat-action"
...
git add $PKG_FILE
git commit -m "chore(chat-action): bump version to $NEW_VERSION"

echo "Creating git tag and pushing..."
TAG_NAME="maia-router-chat-action-v$NEW_VERSION"
git tag $TAG_NAME
