...
export const getVideoProperties = (): INodeProperties[] => [
    { displayName: 'Operation', name: 'operation', type: 'options', noDataExpression: true, displayOptions: { show: { resource: ['video'] } }, options: [ { name: 'Generate Video', value: 'textToVideo', action: 'Generate video from text', description: 'Generate video from text prompt using AI video models' } ], default: 'textToVideo' },
    { displayName: 'Mode', name: 'videoMode', type: 'options', noDataExpression: true, displayOptions: { show: { resource: ['video'], operation: ['textToVideo'] } }, options: [ { name: 'Start', value: 'start', description: 'Start a video generation job and return its ID' }, { name: 'Check Status', value: 'status', description: 'Check the status of a previously started job' }, { name: 'Download', value: 'download', description: 'Download the completed video' } ], default: 'start', description: 'Select how this node should behave to pair with the Wait node' },
    { displayName: 'Model', name: 'videoModel', type: 'options', displayOptions: { show: { resource: ['video'], operation: ['textToVideo'] } }, options: [ { name: 'OpenAI Sora 2', value: 'sora-2' }, { name: 'OpenAI Sora 2 Pro', value: 'sora-2-pro' }, { name: 'Veo 3.0', value: 'veo-3.0-generate-001' } ], default: 'sora-2', required: true, description: 'The video generation model to use' },
    { displayName: 'Prompt', name: 'prompt', type: 'string', typeOptions: { rows: 4 }, displayOptions: { show: { resource: ['video'], operation: ['textToVideo'], videoMode: ['start'] } }, default: '', required: true, description: 'The text prompt describing the video to generate' },
    { displayName: 'Video ID', name: 'videoId', type: 'string', displayOptions: { show: { resource: ['video'], operation: ['textToVideo'], videoMode: ['status','download'], videoModel: ['sora-2', 'sora-2-pro'] } }, default: '', description: 'OpenAI video ID returned from Start (used for Sora models only)', hint: 'Use this field only for OpenAI Sora models' },
    { displayName: 'Operation Name', name: 'operationName', type: 'string', displayOptions: { show: { resource: ['video'], operation: ['textToVideo'], videoMode: ['status','download'], videoModel: ['veo-3.0-generate-001'] } }, default: '', description: 'Vertex AI operation name returned from Start (used for Veo models)', placeholder: 'projects/.../operations/...', hint: 'Required for Veo models when checking status or downloading' },
    { displayName: 'Additional Fields', name: 'videoAdditionalFields', type: 'collection', placeholder: 'Add Field', default: {}, displayOptions: { show: { resource: ['video'], operation: ['textToVideo'] } }, options: [ { displayName: 'Size', name: 'size', type: 'options', options: [ { name: '1280x720', value: '1280x720' }, { name: '1920x1080', value: '1920x1080' }, { name: '720x1280', value: '720x1280' }, { name: '1080x1920', value: '1080x1920' } ], default: '1280x720' }, { displayName: 'Duration (Seconds)', name: 'seconds', type: 'number', default: 8 }, { displayName: 'Sample Count', name: 'sampleCount', type: 'string', default: '1' }, { displayName: 'Project ID', name: 'projectId', type: 'string', default: 'learned-nimbus-473801-q8' }, { displayName: 'Location', name: 'location', type: 'string', default: 'global' }, { displayName: 'Resume URL', name: 'resumeUrl', type: 'string', default: '' } ] },
];
...
export async function executeVideo(ctx: IExecuteFunctions, i: number, returnData: INodeExecutionData[]): Promise<void> {
    ...
    if (mode === 'start') {
        if (isOpenAI) {
            ...
            const createResponse = await requestWithHandling(ctx, { method: 'POST', headers: { 'Authorization': `Bearer ${credentials.apiKey}` }, body, url: 'https://api.maiarouter.ai/openai/v1/videos', json: true } as IHttpRequestOptions);
        ...
        const prompt = ctx.getNodeParameter('prompt', i) as string;
        const projectId = 'learned-nimbus-473801-q8';
    ...
    if (mode === 'status') {
        if (isOpenAI) {
            ...
            const statusResponse = await requestWithHandling(ctx, { method: 'GET', headers: { 'Authorization': `Bearer ${credentials.apiKey}` }, url: `https://api.maiarouter.ai/openai/v1/videos/${videoId}`, json: true } as IHttpRequestOptions);
    ...
    if (mode === 'download') {
        if (isOpenAI) {
            ...
            const videoBuffer = await requestWithHandling(ctx, { method: 'GET', headers: { 'Authorization': `Bearer ${credentials.apiKey}` }, url: `https://api.maiarouter.ai/openai/v1/videos/${videoId}/content`, encoding: 'arraybuffer' } as IHttpRequestOptions);
